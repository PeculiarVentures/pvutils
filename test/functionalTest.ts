...
context('Functional Tests for "pvutils" package', () => {
  ...
  it("getParametersValue", () => {
    assert.equal(pvutils.getParametersValue(1 as any, "name", 2), 2, "Incorrect return value #1");
    assert.equal(
      pvutils.getParametersValue({ name: 33 }, "name", 2),
      33,
      "Incorrect return value #2",
    );
    assert.equal(
      pvutils.getParametersValue({ name: 33 }, "fake", 2),
      2,
  ...
  it("bufferToHexCodes", () => {
    assert.equal(
      pvutils.bufferToHexCodes(data.buffer),
      "0102030405060708090A",
      "Incorrect return value #1",
    );
    assert.equal(
      pvutils.bufferToHexCodes(data.buffer, 1),
      "02030405060708090A",
      "Incorrect return value #2",
    );
    assert.equal(
      pvutils.bufferToHexCodes(data.buffer, 1, 3),
      "020304",
      "Incorrect return value #3",
    );
    assert.equal(
      pvutils.bufferToHexCodes(data.buffer, 1, 3, true),
      "02 03 04",
      "Incorrect return value #3",
    );
  });
  ...
  it("utilFromBase", () => {
    ...
    assert.equal(
      pvutils.utilFromBase(new Uint8Array([0x01, 0x01, 0x01]), 7),
      16513,
      "Incorrect result #3",
    );
  });
  ...
  it("utilToBase", () => {
    ...
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilToBase(129, 7)),
      "0101",
      "Incorrect result #2",
    );
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilToBase(16513, 7)),
      "010101",
      "Incorrect result #3",
    );
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilToBase(16513, 7, 4)),
      "00010101",
      "Incorrect result #4",
    );
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilToBase(16513, 7, 0)),
      "",
      "Incorrect result #5",
    );
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilToBase(16777218, 3)),
      "",
      "Incorrect result #6",
    );
  });
  ...
  it("utilConcatBuf", () => {
    ...
    assert.equal(
      ...
      "0102030405060708090A0102030405060708090A0102030405060708090A",
      "Incorrect return value #3",
    );
  });
  ...
  it("utilEncodeTC", () => {
    assert.equal(
      pvutils.bufferToHexCodes(pvutils.utilEncodeTC(32639)),
      "7F7F",
      "Invalid result #1",
    );
    assert.equal(
  ...
  it("isEqualBuffer", () => {
    ...
    assert.equal(
      pvutils.isEqualBuffer(data.buffer, new Uint8Array().buffer),
      false,
      "Incorrect result #2",
    );
    assert.equal(
  ...
  it("fromBase64", () => {
    assert.equal(
      pvutils.fromBase64("AQIDBAUGBwj//w=="),
      "\x01\x02\x03\x04\x05\x06\x07\x08\xFF\xFF",
      "Incorrect result #1",
    );
    assert.equal(
      pvutils.fromBase64("AQIDBAUGBwj__w==", true),
      "\x01\x02\x03\x04\x05\x06\x07\x08\xFF\xFF",
      "Incorrect result #2",
    );
    assert.equal(
      pvutils.fromBase64("AQIDBAUGBwj__wAA", true, true),
      "\x01\x02\x03\x04\x05\x06\x07\x08\xFF\xFF",
      "Incorrect result #3",
    );
    assert.equal(
      pvutils.fromBase64("AQIDBAUGBwj__w==", true, true),
      "\x01\x02\x03\x04\x05\x06\x07\x08\xFF\xFF",
      "Incorrect result #4",
    );
  ...
  it("arrayBufferToString", () => {
    assert.equal(
      pvutils.arrayBufferToString(data.buffer),
